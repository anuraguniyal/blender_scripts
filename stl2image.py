...
def set_camera_lights(target, obj_rot):
    ...
    cam_move = max(target.dimensions)*1.4
    obj_rot = math.radians(obj_rot) # how much to rotate object
    if target.dimensions.x == min_d:
        cam_location = (cam_move, 0, 0)
        lamp_location = (cam_move*1.5, 0, 0)
        cam_rotation = (0.0, math.radians(90), 0.0)
        target_rotation = (0.0, 0.0, obj_rot)
        ...
        target_rotation = (obj_rot, 0.0, 0.0)
...
def import_stl(stlfile):
    ...
    """
    # before importing lets delete Cube generally found in blender default projects
    try:
...
obj_rot = int(os.environ.get('OBJ_ROT', 45))
stlfile = sys.argv[-2]
imagefile = sys.argv[-1]

target = import_stl(stlfile)
set_camera_lights(target, obj_rot)
